...
> [!TIP] AE2 Analogy: Protocols
> 
> - **HTTP/FTP (Application):** Accessing items via a **Wireless Terminal**. You "Get" items or "Put" items.
> - **TCP (Transport):** **Crafting Monitor**. It tracks the job. If a CPU cancels or resources are missing, it *tells you*. It guarantees you know the result.
> - **UDP (Transport):** **Export Bus**. It dumps items into a machine/chest. If the inventory is full, the items pop off and disappear. It doesn't check if they arrived safetly; it just sends them fast.
> - **IP (Internet):** **Channels**. Finding the path through the dense cables to reach the destination drive.
...
| Feature         | Connection-Oriented (TCP)                                                                    | Connection-Less (UDP)                                         |
|:--------------- |:-------------------------------------------------------------------------------------------- |:------------------------------------------------------------- |
| **Analogy**     | **Phone Call**                                                                               | **Snail Mail / Letter**                                       |
| **Setup**       | **3-Way Handshake:** "Hello? Can you hear me?" $\to$ "Yes, I hear you." $\to$ "Okay, talking now." | **None:** Just throw the letter in the mailbox.               |
| **Reliability** | **Guaranteed:** If a packet is lost, it is re-sent.                                          | **Best Effort:** If it gets lost, it's gone. No confirmation. |
| **Order**       | **Ordered:** Packets arrive in sequence (1, 2, 3...).                                        | **Unordered:** Packets can arrive in any order (3, 1, 2...).  |
| **Speed**       | Slower (due to overhead).                                                                    | Faster (low overhead).                                        |
| **Usage**       | Web browsing (HTTP), Email (SMTP), File Transfer (FTP).                                      | Video Streaming, Gaming, DNS.                                 |
...
> [!NOTE] Switching
> Data transfer across networks often involves [[08 Network Switching|Switching]].
...
See [[10 Data Link Layer]] for details on framing and error control at this level.

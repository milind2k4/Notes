...
> [!TIP] AE2 Analogy: Auto-Crafting a 64k Storage Cell
> Imagine you order a **64k Storage Cell** from your terminal. The process flows down the layers:
> 
> 1.  **Application (Pattern Terminal):** You (User) click the item in the terminal.
> 2.  **Presentation (Encoded Pattern):** The system translates your request into a Pattern (instruction) the machines understand.
> 3.  **Session (Crafting CPU):** A CPU grabs the job, holding the "session" open until the craft is done.
> 4.  **Transport (Molecular Assembler):** The Assembler breaks the job into steps (segments). It ensures reliability (doesn't lose items).
> 5.  **Network (Controller/P2P):** The system finds the route (channel) to the correct Assembler.
> 6.  **Data Link (Smart Cable):** The cable connects specifically to that Interface/Machine.
> 7.  **Physical (Flux Cable):** The raw power and data move through the glass wires.
...
| **1** | **Physical**     | Bit Stream | Transmission over physical medium.         |
...
- **Core Concepts:** Detailed in [[15 Network Layer]].
...
- **Switching:** Using various [[08 Network Switching|Switching Techniques]] to move data.
...
- **Core Concepts:** Detailed in [[10 Data Link Layer]], [[12 Multiple Access Protocols]], and [[14 Ethernet]].
